...
class XtreamClient {
    ...
    async _rebuildLive() {
        if (this._rebuildLock) return;
        this._rebuildLock = true;
        console.log(`[${this.name} SQLite] Iniciando rebuild de canais ao vivo...`);
        try {
            if (!this.authenticated) await this.authenticate();
            
            const categories = await this.getLiveCategories();
            await this.db.run("DELETE FROM live");

            const insert = async (batchData) => {
                if (batchData.length === 0) return;
                await this.db.run("BEGIN TRANSACTION");
                const stmt = "INSERT INTO live (clean_title, stream_id, original_name, category_id, stream_icon) VALUES (?, ?, ?, ?, ?)";
                for (const item of batchData) {
                    await this.db.run(stmt, [item.clean, item.id, item.name, item.cat, item.icon]);
                }
                await this.db.run("COMMIT");
            };

            let batch = [];
            const processLive = (item) => {
            ...
            if (!categories || categories.length === 0) {
                ...
                for (const cat of categories) {
                    const liveStreams = await this.getLiveStreams(cat.category_id);
            ...
            await insert(batch);
            await this.db.run("INSERT OR REPLACE INTO meta(key, value) VALUES ('live_updated', ?)", [String(Date.now())]);
            console.log(`[${this.name} SQLite] Rebuild de canais ao vivo finalizado.`);
        } catch (e) {
            console.error(`[${this.name} SQLite] Erro no rebuild de canais ao vivo:`, e);
        } finally {
            this._rebuildLock = false;
        }
    }
    ...
    async _ensureCatalog(type) {
        await this.initDb();
        const key = type === 'movie' ? 'movies_updated' : type === 'series' ? 'series_updated' : 'live_updated';
        ...
        if (age === 0 || diff > CACHE_TTL) {
            if (!this._rebuildLock) {
                ...
                const task = type === 'movie' ? this._rebuildMovies() : type === 'series' ? this._rebuildSeries() : this._rebuildLive();
...
function initProviders() {
    const figgs = new XtreamClient('Figgs', 'figgs_catalog.db', process.env.FIGS_SERVER, process.env.FIGS_USER, process.env.FIGS_PASS);
...
async function ensureAllCatalogsSequentially() {
    console.log("[STARTUP-PREWARM] Iniciando rebuild sequencial de catálogos no HFA...");
    for (const p of providers) {
        if (!p.enabled) continue;
        
        await p.initDb();
        const ageMovie = await p._getCatalogAge('movies_updated');
        if (ageMovie === 0 || (Date.now() - ageMovie) > 604800000) { // CACHE_TTL reference (7 days)
            await p._rebuildMovies();
        }
        
        const ageSeries = await p._getCatalogAge('series_updated');
        if (ageSeries === 0 || (Date.now() - ageSeries) > 604800000) { // CACHE_TTL reference
            await p._rebuildSeries();
        }
    }
    console.log("[STARTUP-PREWARM] Todos os pre-warms de catálogo concluídos sequencialmente!");
}

module.exports = { searchAllProviders, initProviders, ensureAllCatalogsSequentially };
