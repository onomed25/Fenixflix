...
async def _rebuild_movies(client: MegaplayClient, db: aiosqlite.Connection):
    ...
    print("[Hypex SQLite] Iniciando rebuild de filmes categoria por categoria...")
    try:
        ...
        if not categories:
            print("[Hypex SQLite] Nenhuma categoria encontrada, tentando carregar tudo de uma vez...")
            ...
            print(f"[Hypex SQLite] Processando {len(categories)} categorias de filmes...")
        ...
        await db.commit()
        print(f"[Hypex SQLite] Rebuild concluído. {inserted_count} filmes salvos no SQLite.")
    except Exception as e:
...
async def _rebuild_series(client: MegaplayClient, db: aiosqlite.Connection):
    ...
    print("[Hypex SQLite] Iniciando rebuild de séries categoria por categoria...")
    try:
        ...
        if not categories:
            print("[Hypex SQLite] Nenhuma categoria de série encontrada, tentando carregar tudo de uma vez...")
            ...
            print(f"[Hypex SQLite] Processando {len(categories)} categorias de séries...")
            for cat in categories:
                cat_id = cat.get("category_id")
                if not cat_id:
                    continue
        ...
        await db.commit()
        print(f"[Hypex SQLite] Rebuild concluído. {inserted_count} séries salvas no SQLite.")
    except Exception as e:
